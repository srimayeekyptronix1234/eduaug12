...
<?php
$school_id = school_id();
$marks = $this->crud_model->get_quiz_mark($exam_id, $class_id, $section_id, $subject_id)->result_array();
    ...
    <table class="table table-bordered table-responsive-sm" width="100%">
        ...
        <?php foreach($marks as $mark):
                ...
                <tr>
                    ...
                    <td class="text-center"><button class="btn btn-success" onclick="quiz_mark_update('<?php echo $mark['student_id']; ?>')"><i class="mdi mdi-checkbox-marked-circle"></i></button>|<button class="" onclick="view_quiz_mark('<?php echo $school_id; ?>','<?php echo $mark['student_id']; ?>','<?php echo $class_id; ?>','<?php echo $subject_id; ?>','<?php echo $exam_id; ?>')"><?php echo get_phrase('mark'); ?></button>
                    </td>
                </tr>
        <?php endforeach; ?>
        </tbody>
...
<!----- Modal code To view mark-----> 
<div class="modal fade" id="viewMarksModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Quarter Set wise marks</h5>
      </div>
      <div class="modal-body view_mark_content">
...
<script>
  // View Quiz Mark
  function view_quiz_mark(school_id,student_id,class_id,subject_id,exam_id){
        if(subject_id != ""){
            $.ajax({
                type : 'POST',
                url : '<?php echo route('quiz_marks/view_marks'); ?>',
                data : {school_id : school_id, student_id : student_id, class_id : class_id, subject_id : subject_id, exam_id : exam_id},
                success : function(response){
                    $('.view_mark_content').html(response);
    ...
    function closeModal() {
      $('#viewMarksModal').modal('hide');
    }

  function quiz_mark_update(student_id){
        ...
        if(subject_id != ""){
            $.ajax({
                type : 'POST',
                url : '<?php echo route('quiz_marks/quiz_mark_update'); ?>',
                ...
                success : function(response){
                    success_notify('<?php echo get_phrase('quiz_mark_has_been_updated_successfully'); ?>');
                }
            });
        }else{
            toastr.error('<?php echo get_phrase('required_mark_field'); ?>');
